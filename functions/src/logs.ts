...
export const unsupportedType = (unsupportedTypes: string[], contentType: string) => {
  logger.log(`Image type '${contentType}' is not supported, here are the supported file types: ${unsupportedTypes.join(', ')}`);
};
...
export const imageOutsideOfPaths = (absolutePaths: string[], imagePath: string) => {
  logger.log(`Image path '${imagePath}' is not supported, these are the supported absolute paths: ${absolutePaths.join(', ')}`);
};

export const imageInsideOfExcludedPaths = (absolutePaths: string[], imagePath: string) => {
  logger.log(`Image path '${imagePath}' is not supported, these are the not supported absolute paths: ${absolutePaths.join(', ')}`);
};
...
export const imageConverting = (originalImageType: string, imageType: string) => {
  logger.log(`Converting image from type, ${originalImageType}, to type ${imageType}.`);
};
